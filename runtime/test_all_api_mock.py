...
def build_url(path):
    # urllib 不能直接请求带中文的 URL，这里把中文 query 自动转成百分号编码
    if "?" in path:
    ...
    try:
        with urllib.request.urlopen(req, timeout=8) as resp:
            ...
            try:
                ...
                parsed = text[:120]
                is_json = False
            return resp.status, is_json, parsed
...
login_status, login_json, login_resp = request_api(
    ...
    body={
        "code": "test_code_all_api",
        "encryptedData": "demo_encrypted",
        "iv": "demo_iv",
        "nickname": "接口测试用户"
    }
)
...
tests = [
    ...
    ("DELETE", "/api/play-history/delete", {"historyId": "1"}),
...
for method, path, body in tests:
    ...
    ok = 200 <= status < 300 and is_json

    # /api/player/add-next 如果同一首歌已经在下一首队列中，按接口设计会返回 409
    # 这是正常业务结果，不算接口失败。
    ...
    print("{:<8} {:<55} {:<8} {:<8} {}".format(
        method,
        path[:55],
        status,
        "YES" if is_json else "NO",
        msg
    ))
