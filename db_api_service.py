...
FRONT_DATA_CATALOG.extend([
    ...
    {"group": "决策报表", "title": "运营日报列表", "frontend": "决策报表 / reports", "api": "/api/admin/super/reports", "table": "daily_stats", "fields": ["stat_date", "total_play_count", "unique_user_count"], "operation": "报表由最近 daily_stats 行生成；改日报行会改变报表名称、摘要和原始数据。"},
    {"group": "营销洞察", "title": "新增-绑定-首播-7日活跃漏斗", "frontend": "营销洞察 / funnels", "api": "/api/admin/market/insights", "table": "user", "fields": ["user_id", "created_at"], "operation": "漏斗基准是近 30 天新增用户；绑定来自 user_device_binding，首播和 7 日活跃来自 play_history。比例后端封顶到 100%。"},
    {"group": "营销洞察", "title": "绑定设备漏斗步骤", "frontend": "营销洞察 / 绑定设备", "api": "/api/admin/market/insights", "table": "user_device_binding", "fields": ["user_id", "device_id", "bind_time"], "operation": "近 30 天新增用户中存在绑定记录的人会进入绑定设备步骤。"},
    {"group": "营销洞察", "title": "首播 / 7日活跃漏斗步骤", "frontend": "营销洞察 / 首播与活跃", "api": "/api/admin/market/insights", "table": "play_history", "fields": ["user_id", "created_at", "play_duration"], "operation": "近 30 天新增用户中有播放记录算完成首播；最近 7 天仍有播放算 7 日活跃。"},
    ...
    {"group": "设备固件", "title": "当前固件版本 / 最新版本", "frontend": "设备固件 / firmwareVersion", "api": "/api/admin/operator/device/firmware-version", "table": "device", "fields": ["device_id", "firmware_version"], "operation": "当前版本来自 device.firmware_version；最新版本来自 device_firmware 或环境默认值。"},
    {"group": "设备固件", "title": "固件包列表", "frontend": "设备固件 / firmwarePackages", "api": "/api/admin/operator/device/firmware-packages", "table": "device_firmware", "fields": ["firmware_id", "model_name", "version", "version_code", "file_size", "status"], "operation": "只显示 status 不是 deleted/disabled 的固件包；version_code 越大越靠前。"},
    {"group": "设备固件", "title": "固件升级任务", "frontend": "任务中心 / firmwareTasks", "api": "/api/admin/operator/device/firmware-tasks", "table": "device_firmware_update_task", "fields": ["task_no", "device_id", "current_version", "target_version", "status", "progress", "fail_reason"], "operation": "任务列表来自 device_firmware_update_task；新建灰度任务会新增一行 pending 任务。"},
