...
FRONT_DATA_CATALOG.extend([
    ...
    {"group": "系统账号与权限", "title": "后台登录与最近登录", "frontend": "登录页、个人信息、用户管理最近登录", "api": "/api/admin/login；/api/admin/captcha；/api/admin/login-code；/api/admin/sms-code；/api/admin/profile", "table": "admin_user", "targetTable": "admin_user", "fields": ["username", "password_hash", "last_login_at", "status"], "operation": "登录成功会更新 admin_user.last_login_at；登录页提供账号密码和模拟短信验证码两种方式，二者都需先完成 ALTCHA proof-of-work 机器人验证；账号密码登录需要四位登录验证码，短信验证码登录不再需要四位验证码；短信手机号只校验中国大陆手机号格式，验证码只校验 6 位数字，未匹配后台账号时回退默认 admin；默认账号没有数据库行时，会从后端默认管理员配置回显。"},
...
FRONT_DATA_CATALOG = [
    ...
    {"group": "数据总览", "title": "活跃度 / 活跃用户", "roles": ["super_admin", "boss", "market_admin"], "frontend": "数据总览 / 指标卡片", "api": "/api/admin/super/overview/activity-rate", "table": "user_profile", "targetTable": "user_profile", "fields": ["active_level", "user_id"], "operation": "活跃用户来自 user_profile.active_level='high'。"},
    ...
    {"group": "数据总览", "title": "市场画像维度", "roles": ["market_admin"], "frontend": "数据总览 / 画像维度卡片", "api": "/api/admin/market/user-profile/age-distribution；/api/admin/market/user-profile/region-distribution", "table": "user_profile", "targetTable": "user_profile", "fields": ["age_range", "province_name"], "operation": "市场管理员数据总览的画像维度来自年龄与地区画像分布。"},
    {"group": "数据总览", "title": "市场运营人群", "roles": ["market_admin"], "frontend": "数据总览 / 运营人群卡片", "api": "/api/admin/market/segments", "table": "user_value_segment_daily", "targetTable": "user_profile", "fields": ["value_level", "active_level"], "operation": "市场管理员数据总览的运营人群来自用户分群日报。"},
