...
FRONT_DATA_CATALOG.extend([
    ...
    {"group": "设备固件", "title": "历史升级记录", "frontend": "告警中心 / 审计日志", "api": "/api/admin/operator/device/alerts；/api/admin/super/security/logs", "table": "device_firmware_update_task", "fields": ["task_no", "device_id", "current_version", "target_version", "status", "progress", "fail_reason"], "operation": "历史升级记录仅用于告警和审计；前端不再提供独立列表或新建入口。"},
...
FRONT_DATA_CATALOG.extend([
    ...
    {"group": "设备固件与历史记录", "title": "历史升级记录", "frontend": "告警中心 / 审计日志", "api": "/api/admin/operator/device/alerts；/api/admin/super/security/logs", "table": "device_firmware_update_task", "targetTable": "device_firmware_update_task", "fields": ["task_id", "task_no", "device_id", "current_version", "target_version", "status", "progress", "fail_reason", "created_at"], "operation": "历史升级记录仅用于告警和审计核验；前端不再提供独立列表或新建入口。"},
