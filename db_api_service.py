...
FRONT_DATA_CATALOG.extend([
    ...
    {"group": "系统账号与权限", "title": "后台登录与最近登录", "frontend": "登录页、个人信息、用户管理最近登录", "api": "/api/admin/login；/api/admin/sms-code；/api/admin/profile", "table": "admin_user", "targetTable": "admin_user", "fields": ["username", "password_hash", "last_login_at", "status"], "operation": "登录成功会更新 admin_user.last_login_at；登录页提供账号密码和模拟短信验证码两种方式，二者都需先完成机器人验证；短信手机号只校验中国大陆手机号格式，验证码只校验 6 位数字，未匹配后台账号时回退默认 admin；默认账号没有数据库行时，会从后端默认管理员配置回显。"},
