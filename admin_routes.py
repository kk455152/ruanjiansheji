...
def device_logs():
    ...
    result = synthetic_device_log_rows()
    result = sorted(result, key=lambda item: item.get("createdAt") or "", reverse=True)
    start = (page - 1) * page_size
    visible = result[start:start + page_size]
    return response_ok({"total": len(result), "page": page, "pageSize": page_size, "list": visible})
...
def synthetic_device_log_rows():
    ...
    for row in device_rows:
        ...
        rows.append({
            ...
            "logLevel": "info",
            "title": "最近活跃时间",
            "content": f"最近活跃时间：{created_at or '-'}",
            "eventCode": "DEVICE_RECENT_ACTIVITY",
...
def synthetic_device_log_detail(log_id):
    ...
    if text.startswith("device-state-"):
        ...
        return {
            ...
            "onlineStatusText": row.get("online_status") or row.get("status") or "",
