...
def _conn():
    conn = mysql_conn()
    if conn is None:
        raise RuntimeError("MySQL 连接失败：请检查 db.py 或 MYSQL_USER/MYSQL_PASSWORD/MYSQL_DATABASE 环境变量")
    return conn
...
def save_player_volume(device_id, volume, body=None):
    ...
    _exec("""
    INSERT INTO api_device_runtime
    (device_id, volume, updated_at)
    VALUES (%s,%s,%s)
    ON DUPLICATE KEY UPDATE
